...
def get_word_phone_list(word_dict, word_list):
    ...
    char_index = 0
    word_index = 0
    lex_dict = mld.get_lexicon_dict('./dicts/lexicon.txt')
...
if training_data:
    ...
    text_file = './inp/raw_text.txt'
    label_phone_align = './label_phone_align/'
else:
...
for text_line in textlines:
    ...
    if training_data:
        ...
        ali_file = './cup_ali/' + sent_index + '.txt'
        with open(ali_file, 'r') as fid:
            ali_file_lines = fid.readlines()
        for line in ali_file_lines:
            line = line.strip()
    ...
    for index in range(iter_len):
        ...
        if c_phone != 'sil':
            ...
            if index == 0:
                ...
                next_pos = pos_list[next_word_index]
        if training_data:
            label_phone_fid.write("{25} {26} {0}^{1}-{2}+{3}={4}@{5}_{6}/A:{7}_{8}/B:{9}_{10}!{11}_{12}/C:{13}+{14}/D:{15}-{16}/E:{17}&{18}^{19}_{20}/F:{21}_{22}/G:{23}-{24}\n".format(ll_phone,l_phone,c_phone,r_phone,rr_phone,
                                                            ...
                                                            str(fw_word_utt), str(bw_word_utt),next_pos,next_word_num,str(all_syl_len),str(len(word_list)), start_time_list[index], end_time_list[index]))
        else:
